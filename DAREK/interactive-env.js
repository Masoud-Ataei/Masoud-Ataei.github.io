...
const rect_poly = document
  .getElementById("polynomial_line")
  .querySelector("rect");
...
function updatePath() {
  ...
  if (vis_poly) {
    ...
    f +=
      upper.map((p) => `${p[0]},${p[1]}`).join(" ") +
      " " +
      lower.map((p) => `${p[0]},${p[1]}`).join(" ") +
      " Z";
    fillNP.setAttribute("d", f);
  } else {
    fillNP.setAttribute("d", '');
    pathNP.setAttribute("d", '');
  }
  ...
  if (vis_gp) {
    ...
    for (let i = 0; i < mean.length; i++) {
      upperPoints.push(
        `${i === 0 ? "M" : "L"} ${xs[i]},${mean[i] + std[i] * 100}`
      );
      lowerPoints.push(
        `L ${xs[mean.length - 1 - i]},${
          mean[mean.length - 1 - i] - std[mean.length - 1 - i] * 100
        }`
      );
    }
    ...
    fillGP.setAttribute("d", '');   // or ''
    pathGP.setAttribute("d",'');
  }
}
...
rect_gp.addEventListener("click", () => {
  // Toggle between two colors
  if (rect_gp.getAttribute("fill") === "#ff7777") {
    rect_gp.setAttribute("fill", "#ffffff"); // change to white
  } else {
    rect_gp.setAttribute("fill", "#ff7777"); // change back to blue
  };
  updatePath();
});
